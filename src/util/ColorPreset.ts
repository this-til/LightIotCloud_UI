...
export const colorPresetMap: Record<string, Record<string, Color>> = {
  "车祸": {
    "车祸": [255, 0, 0]
  },
  "倒地": {
...
const generatedColorCache: Record<string, Record<string, Color>> = {};

/**
 * Generate a random bright color (high saturation and value)
 * @returns RGB color as [number, number, number]
 */
function generateRandomBrightColor(): Color {
  // Generate hue (0-360)
 ...
 * @returns RGB color as [number, number, number]
 */
export function getColorPreset(modelName: string, itemName: string): Color {
