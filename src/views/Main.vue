...
const particlesOptions = {
  fullScreen: {                    // 由我们自己定位，禁用默认全屏
    enable: false
  },
  background: {                    // 透明背景，方便叠到界面上
...
onMounted(() => {
  ...
  unsubscribeDeviceOnlineStateSwitchEvent =
    subscriptionDeviceOnlineStateSwitchEvent(getDefWebSocketClient(), {
      next(value) {
        ElNotification({
          type:
            value.onlineState === OnlineState.ONLINE
              ? 'success'
              : 'warning',
          title:
            value.onlineState === OnlineState.ONLINE
...
#tsparticles+canvas {
  /* 保险：同时覆盖生成的 <canvas> */
  z-index: 2 !important;
  filter: drop-shadow(0 0 6px #00ffff) blur(.4px) brightness(300%);
  opacity: 1;
...
.particles.dim + canvas {
  /* 同步降低亮度 + 半透明 */
  filter: drop-shadow(0 0 6px #00ffff) blur(.4px) brightness(100%);
  opacity: .3;
}
