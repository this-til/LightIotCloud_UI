...
import { login, setToken } from '@/util/Api.js'
//初始化变量
const username = ref('')
const password = ref('')
const error = ref()
  ...
  <div class="big-box">
    <div class="mid-box">
      <div class="login-box" v-if="currentView === 'index'">
        ...
        <div class="input-container">
          <input type="password" required="required" v-model="password" @keydown.enter="loginTab" />
          <span>密码</span>
          <i></i>
        </div>
        <div style="color: red; margin-top: 10px;">{{ error }}</div>
        <input type="submit" value="登录" @click="loginTab" />
        <div class="bottom-right">
          <p class="getPassworde" @click="tabChoose('recover')">找回密码</p>
          <p class="registered" @click="tabChoose('registered')">注册</p>
        </div>
      </div>
      <component v-if="currentView !== 'index'" :is="currentViews[currentView]" @tabView="tabChoose" />
    </div>
  </div>
</template>
